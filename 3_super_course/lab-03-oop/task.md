...
переопределить calculateSalary()
(зарплата = базовая + переработка × фиксированная ставка)
...
Алгоритма создания и добавления сотрудников
...
Почему массив объявляется как Employee[], а не Manager[]?

Какие преимущества даёт ООП по сравнению с процедурным подходом?